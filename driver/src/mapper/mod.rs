...
pub unsafe fn manually_map(module_base: *mut u8) -> Option<i32> {
    ...
    let ntoskrnl_base = get_module_base(b"ntoskrnl.exe");
    log::info!("[+] ntostrnl.exe base address: {:p}", ntoskrnl_base);

    // Resolve imports using ntoskrnl
    resolve_imports(new_module_base, ntoskrnl_base as _).expect("Failed to resolve imports");
