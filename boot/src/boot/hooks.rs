...
use crate::boot::globals::{ALLOCATED_BUFFER, DRIVER_IMAGE_SIZE, TARGET_DRIVER_HASH, NTOSKRNL_HASH, BlImgAllocateImageBufferSignature_1, BlImgAllocateImageBufferSignature_2, ORIGINAL_BYTES, BL_MEMORY_ATTRIBUTE_RWX, ImgArchStartBootApplicationSignature, OslFwpKernelSetupPhase1Signature_1, OslFwpKernelSetupPhase1Signature_2};
...
use super::globals::{JMP_SIZE, ORIGINAL_BYTES_COPY, BL_MEMORY_TYPE_APPLICATION};
...
pub fn setup_hooks(bootmgfw_handle: &Handle, boot_services: &BootServices) -> uefi::Result<> {
    ...
    let offset = pattern_scan(bootmgfw_data, ImgArchStartBootApplicationSignature).expect("Failed to pattern scan").expect("Failed to find ImgArchStartBootApplication signature");

    // Print the bootmgfw.efi image base and of ImgArchStartBootApplication offset and image base
    log::info!("[+] bootmgfw.efi {:#p} + ImgArchStartBootApplication offset {:#x} = {:#x}", image_base, offset, (image_base as usize + offset));
...
pub fn img_arch_start_boot_application_hook(_app_entry: *mut u8, image_base: *mut u8, image_size: u32, _boot_option: u8, _return_arguments: *mut u8) -> uefi::Status {
    ...
    let mut offset = pattern_scan(winload_data, OslFwpKernelSetupPhase1Signature_1).expect("Failed to pattern scan");

    if offset.is_none() {
        offset = pattern_scan(winload_data, OslFwpKernelSetupPhase1Signature_2).expect("Failed to pattern scan");
    }
    ...
    log::info!("[+] winload.efi {:#p} + OslFwpKernelSetupPhase1 offset {:#x} = {:#x}", image_base, offset, (image_base as usize + offset));
    ...
    let mut offset = pattern_scan(winload_data, BlImgAllocateImageBufferSignature_1).expect("Failed to pattern scan");

    if offset.is_none() {
        offset = pattern_scan(winload_data, BlImgAllocateImageBufferSignature_2).expect("Failed to pattern scan");
    }
